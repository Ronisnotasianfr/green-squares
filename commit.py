...
min_total = 3
max_total = 15# Load or initialize tracking
...
# Random number of commits for this hour
slot_commit = random.randint(0, 2)
slot_commit = min(slot_commit, remaining)

# Ensure min_total is met by end of day
if now.hour >= 20 and done + slot_commit < min_total:
    slot_commit = min(min_total - done, remaining)

if slot_commit == 0:
    print("🛌 Nothing to do this hour.")
    exit()

log_entries = []

# Do the commits
for i in range(slot_commit):
    # Random delay before each commit so timestamps are spread out
    if i > 0 or random.random() < 0.5:
        delay_minutes = random.randint(1, 50)
        print(f"😴 Sleeping {delay_minutes}m before commit...")
